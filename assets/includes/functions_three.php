...
function Wo_RegisterPoint($post_id, $type, $action = '+', $user_id = 0)
{
    ...
    if (empty($wo["user"]["point_day_expire"])) {
        ...
        $db->where('user_id', $user_id)->update(
            T_USERS,
            array(
...
function Wo_GetBlogCommentData($id)
{
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            ...
            $fetched_data['replies'] = Wo_GetBlogCommentReplies(
                array(
...
function Wo_GetForumSec($args = array())
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            if ($forums) {
                $fetched_data['forums'] = Wo_GetForums(
                    array(
...
function Wo_GetForums($args = array())
{
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        $fetched_data['posts'] = Wo_GetTotalThreads(
            array(
...
function Wo_GetForumInfo($fid)
{
    ...
    if (mysqli_num_rows($sql_query_one)) {
        while ($fetched_data = mysqli_fetch_assoc($sql_query_one)) {
            $data['threads'] = Wo_GetForumThreads(
                array(
                    "forum" => $fetched_data['id'],
                    "limit" => 10
                )
...
function Wo_GetForumThreads($args = array())
{
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql_queryset)) {
        ...
        if (empty($args['threadreplies'])) {
            $fetched_data['threadreplies'] = Wo_GetThreadReplies(
                array(
                    "thread_id" => $fetched_data['id']
                )
            );
        }
        $fetched_data['replies'] = Wo_GetTotalReplies(
            array(
...
function Wo_GetForumUsers($args = array())
{
    ...
    if (mysqli_num_rows($sql)) {
        while ($fetched_data = mysqli_fetch_assoc($sql)) {
            ...
            $data_user['forum_posts'] = Wo_GetTotalThreads(
                array(
...
function Wo_GetMovieCommentData($id)
{
    ...
    if (mysqli_num_rows($query)) {
        ...
        if (!empty($fetched_data)) {
            ...
            $fetched_data['replies'] = Wo_GetMovieCommentReplies(
                array(
                    'comm_id' => $fetched_data['id']
                )
            );
...
function Wo_GroupTabData($id = false, $update_seen = true)
{
    ...
    if ($query && mysqli_num_rows($query) > 0) {
        ...
        $data['messages'] = Wo_GetGroupMessages(
            array(
...
function Wo_MarkAllChatsAsRead($user_id = 0)
{
    ...
    $update = $db->where('to_id', $user_id)->update(
        T_MESSAGES,
        array(
            'seen' => time()
        )
...
function Wo_TwoFactor($username = '', $id_or_u = 'user')
{
    ...
    $hash_code = md5($code);
    $update_code = $db->where('user_id', $getuser['user_id'])->update(
        T_USERS,
        array(
...
function Wo_VerfiyIP($username = '')
{
    ...
    if ($getIpInfo['status'] == 'success' && !empty($getIpInfo['regionName']) && !empty($getIpInfo['countryCode']) && !empty($getIpInfo['timezone']) && !empty($getIpInfo['city'])) {
        ...
        if ($create_new == true) {
            ...
            $update_user = $db->where('user_id', $getuser['user_id'])->update(
                T_USERS,
                array(
...
function AddNewRef($ref_id, $user_id, $amount)
{
    ...
    for ($i = 0; $i < $wo['config']['affiliate_level']; $i++) {
        $user = $db->where('user_id', $ref_id)->getOne(
            T_USERS,
            array(
                'referrer',
                'ref_level'
        ...
        if (!empty($user->referrer)) {
            ...
            $pre_user_data = $db->where('user_id', $user->referrer)->getOne(
                T_USERS,
                array(
                    'ref_level'
                )
            ...
            $db->where('user_id', $user->referrer)->update(
                T_USERS,
                array(
...
function Wo_Insert_interest($interest_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $length = count($interest_ids['interest_id']);
    for ($i = 0; $i < $length; $i++) {
        $user_id = $wo['user']['user_id'];
        $interest_id = $interest_ids['interest_id'][$i];
        $insert_user_intrest = "INSERT INTO `wo_user_interest`(`user_id`, `interest_id`) VALUES ('$user_id','$interest_id')";
        mysqli_query($sqlConnect, $insert_user_intrest);
    }
    return True;
}

function Wo_delete_interest($user_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $delete_interest_query = "DELETE FROM `wo_user_interest` WHERE `user_id`=$user_id";
    $delete_query_result = mysqli_query($sqlConnect, $delete_interest_query);
...
function Wo_list_interest($user_id, $search = "")
{
    global $sqlConnect, $wo;
    ...
    $response_array = $interest_result = $user_interest_post = array();
    if (isset($user_id)) {
        // all interest list 
        if (!empty($search)) {
            ...
            foreach (range('A', 'Z', ) as $v) {
                $all_interest = "SELECT * FROM `wo_interest` Where `name`   LIKE '$v%' ORDER BY
              `name` LIMIT 2 ";
        ...
        if (mysqli_num_rows($all_interest_response) > 0) {
            while ($data = mysqli_fetch_assoc($all_interest_response)) {
                $all_interest_list[] = $data;
            }
        }

        $list_interest_query = "SELECT ui.*,i.name FROM `wo_user_interest` ui LEFT OUTER JOIN wo_interest i on (ui.interest_id=i.id) where `user_id`=$user_id";
        $list_query_result = mysqli_query($sqlConnect, $list_interest_query);
        if (mysqli_num_rows($list_query_result) > 0) {
            while ($data = mysqli_fetch_assoc($list_query_result)) {
                $interest_result[] = $data;
            ...
            if (mysqli_num_rows($user_interest_response) > 0) {
                while ($user_interest = mysqli_fetch_assoc($user_interest_response)) {
                    $id = $user_interest['id'];
                    $post_count_on_user_interest_query = "SELECT COUNT('*') as post_on_user_interest FROM `wo_knkt_post_interest` pi LEFT OUTER JOIN `wo_knkt_post` p  on (pi.
`post_id`=p.id)   WHERE pi.`interest_id` ='$id' and p.status=1";
                    $post_count_on_user_interest_response = mysqli_query($sqlConnect, $post_count_on_user_interest_query);
                    $post_count = mysqli_fetch_object($post_count_on_user_interest_response);
                    $count = $post_count->post_on_user_interest;
                    $user_interest['post_count'] = $count;
                    $user_interest_post[] = $user_interest;
                }
            }
            return $response_array = array('user_interest' => $interest_result, "interest_data" => $user_interest_post, 'all_interest' => $all_interest_list);
        } else {
            return $response_array = array('user_interest' => $interest_result, "interest_data" => $user_interest_post, 'all_interest' => $all_interest_list);
        }
    } else {
...
function Wo_Edit_interest($interest_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_id = $wo['user']['user_id'];
    $Delete_user_interest = "DELETE FROM `wo_user_interest` WHERE `user_id`=$user_id";
    $delete_interest_response = mysqli_query($sqlConnect, $Delete_user_interest);
    if ($delete_interest_response == 1) {
        $length = count($interest_ids);
        for ($i = 0; $i < $length; $i++) {
            $interest_id = $interest_ids[$i];
            $insert_user_intrest = "INSERT INTO `wo_user_interest`(`user_id`, `interest_id`) VALUES ('$user_id','$interest_id')";
            mysqli_query($sqlConnect, $insert_user_intrest);
        }
        return array('status' => "1", "data" => "User Interest Updated");
    } else {
        return array('status' => "0", "data" => "User Interest Not Updated");
    }
}

function Wo_Insert_Post($post_data)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (empty($post_data['post_title'])) {
        ...
        $post_describtions = $post_data['post_description'];
        $user_id = $post_data['user_id'];
        $location = $post_data['location'];
        $date = date("Y-m-d");
        $insert_post_query = "INSERT INTO `wo_knkt_post`(`post_title`, `post_describtions`, `user_id`, `location`, `delete_after_6_month`, `status`, `date`, `post_like`) VALUES ('$post_title','$post_describtions','$user_id','$location',1,0,'$date',0)";
        $insert_post_result = mysqli_query($sqlConnect, $insert_post_query);
...
function Wo_post_media($id, $media, $media_type = 0)
{
    global $wo, $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    ...
    $post_image_query = "INSERT INTO `wo_knkt_post_media`(`post_id`, `media`,`media_type`) VALUES ('$id','$media',
    '$media_type')";
    $query_one = mysqli_query($sqlConnect, $post_image_query);
    if ($query_one) {
        return true;
    }
}
function Wo_post_interest($post_id, $interest_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    for ($i = 0; $i < $length; $i++) {
        $interest_id = $interest_ids['interest_id'][$i];
        $insert_user_intrest = "INSERT INTO `wo_knkt_post_interest`(`post_id`, `interest_id`)  VALUES ('$post_id','$interest_id')";
        mysqli_query($sqlConnect, $insert_user_intrest);
    }
    return True;
}
function Wo_post_tags($post_id, $user_tag_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $length = count($user_tag_ids['user_tags_id']);
    for ($i = 0; $i < $length; $i++) {
        $user_tag_id = $user_tag_ids['user_tags_id'][$i];
        $insert_user_tags_id_query = "INSERT INTO `wo_knkt_post_tags`(`post_id`, `user_id`, `status`) VALUES ('$post_id','$user_tag_id',1)";
        mysqli_query($sqlConnect, $insert_user_tags_id_query);
    }
    return True;
}
function Wo_get_post_data($post_id)
{

    global $sqlConnect, $wo;
    $login_user_id = $wo['user']['user_id'];
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (!empty($post_id)) {
        ...
        $post_response = "SELECT * FROM `wo_knkt_post` WHERE `id`='$post_id'";
        $post_data_response = mysqli_query($sqlConnect, $post_response);
        $post_data = mysqli_fetch_assoc($post_data_response);
        $user_id = $post_data['user_id'];
        $get_user_query = "SELECT * FROM `Wo_Users` WHERE `user_id`='$user_id'";
        $get_user_result = mysqli_query($sqlConnect, $get_user_query);
        $get_user_data = mysqli_fetch_object($get_user_result);
        if ($post_data) {
            $post_image_response = "SELECT * FROM `wo_knkt_post_media` WHERE `post_id`='$post_id'";
            $post_image_data_response = mysqli_query($sqlConnect, $post_image_response);
            while ($data = mysqli_fetch_assoc($post_image_data_response)) {
                $media_result[] = $data;
            }
            $post_interest_response = "SELECT i.name,i.id FROM `wo_knkt_post_interest` pi RIGHT OUTER JOIN wo_interest i on (pi.interest_id=i.id) WHERE pi.`post_id`='$post_id'";
            $post_interest_data_response = mysqli_query($sqlConnect, $post_interest_response);
            while ($data = mysqli_fetch_assoc($post_interest_data_response)) {
                $interest_result[] = $data;
            }
            $post_user_tag_response = "SELECT u.username as tag_user FROM `wo_knkt_post_tags` pt LEFT OUTER JOIN wo_users u on (pt.user_id=u.user_id) WHERE `post_id`='$post_id'";
            $post_user_tag_data_response = mysqli_query($sqlConnect, $post_user_tag_response);
            while ($data = mysqli_fetch_assoc($post_user_tag_data_response)) {
                $user_tag_result[] = $data;
            }
            $comment_list_query = "SELECT c.* ,u.first_name,u.last_name,u.avatar FROM `wo_knkt_post_comments` c LEFT OUTER JOIN `Wo_Users` u on (c.created_by=u.user_id)  WHERE c.post_id='$post_id'";
            $comment_list_result = mysqli_query($sqlConnect, $comment_list_query);
            if (mysqli_num_rows($comment_list_result) > 0) {
                $comment_list = mysqli_fetch_all($comment_list_result, MYSQLI_ASSOC);
                $post_data['comment_list'] = $comment_list;
            }
            $like_list_query = "SELECT *  FROM `wo_knkt_post_like` WHERE `post_id`='$post_id' and `user_id`='$login_user_id'";
            $like_list_result = mysqli_query($sqlConnect, $like_list_query);
            if (mysqli_num_rows($like_list_result) > 0) {
                $like_data = mysqli_fetch_object($like_list_result);
                $post_data['like_data'] = $like_data;
            }
            $post_follow_query = "SELECT *  FROM `wo_followers`WHERE `following_id`='$user_id' and `follower_id`='$login_user_id'";
            $post_follow_result = mysqli_query($sqlConnect, $post_follow_query);
            if (mysqli_num_rows($post_follow_result) > 0) {
                $post_data['fellow'] = '1';
            } else {
                $post_data['fellow'] = '0';
            }
            $post_data['user_tags'] = $user_tag_result;
            $post_data['user_post_interest'] = $interest_result;
            $post_data['images'] = $media_result;
            $post_data['user_data'] = $get_user_data;
            return $post_data;
        } else {
            return false;
        }
...
function Wo_delete_post($post_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (!empty($post_id)) {
        ...
        $delete_post_response = "DELETE FROM `wo_knkt_post` WHERE `id`='$post_id'";
        $delete_post_response = mysqli_query($sqlConnect, $delete_post_response);
        if ($delete_post_response) {
            // delete post media
            $delete_post_media_query = "DELETE FROM `wo_knkt_post_media` WHERE  `post_id`='$post_id'";
            mysqli_query($sqlConnect, $delete_post_media_query);
            // delete post interest
            $delete_post_interest_query = "DELETE FROM `wo_knkt_post_interest` WHERE`post_id`='$post_id'";
            mysqli_query($sqlConnect, $delete_post_interest_query);
            // delete post tags 
            $delete_post_tags_query = "DELETE FROM `wo_knkt_post_tags` WHERE`post_id`='$post_id'";
            mysqli_query($sqlConnect, $delete_post_tags_query);
            if ($delete_post_tags_query) {
                return true;
            }
        } else {
            return false;
...
function Wo_edit_post($post_data)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (empty($post_data['post_title'])) {
        ...
        $edit_post_query = "UPDATE `wo_knkt_post` SET `post_title`='$post_title',`post_describtions`='$post_describtions',`location`='$location' WHERE `id`=$post_id";
        $edit_post_query = mysqli_query($sqlConnect, $edit_post_query);
        if ($edit_post_query) {
            return true;
        } else {
            return false;
...
function Wo_delete_media($post_id)
{
    global $wo, $sqlConnect;
    // delete post media
    $delete_media_query = "DELETE FROM `wo_knkt_post_media` WHERE  `post_id`='$post_id'";
    $delete_media_response = mysqli_query($sqlConnect, $delete_media_query);
    if ($delete_media_response) {
        return true;
    } else {
        return false;
    }
}
function Wo_edit_media($id, $media, $media_type = 0)
{
    global $wo, $sqlConnect;
    if (empty($id) or !is_numeric($id) or $id < 1) {
        return false;
    ...
    $post_image_query = "INSERT INTO `wo_knkt_post_media`(`post_id`, `media`,`media_type`) VALUES ('$id','$media','$media_type')";
    $query_one = mysqli_query($sqlConnect, $post_image_query);
    if ($query_one) {
        return true;
    }
}
function Wo_edit_post_interest($post_id, $interest_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    // delete post interest
    $delete_post_interest_query = "DELETE FROM `wo_knkt_post_interest` WHERE`post_id`='$post_id'";
    $delete_interest_response = mysqli_query($sqlConnect, $delete_post_interest_query);
    if ($delete_interest_response) {
        for ($i = 0; $i < $length; $i++) {
            $interest_id = $interest_ids['interest_id'];
            $insert_user_intrest = "INSERT INTO `wo_knkt_post_interest`(`post_id`, `interest_id`)  VALUES ('$post_id','$interest_id[$i]')";
            mysqli_query($sqlConnect, $insert_user_intrest);
        }
        return True;
    } else {
...
function Wo_edit_tags($post_id, $user_tag_ids)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $length = count($user_tag_ids['user_tags_id']);
    $delete_post_tags_query = "DELETE FROM `wo_knkt_post_tags` WHERE`post_id`='$post_id'";
    $delete_post_tags_response = mysqli_query($sqlConnect, $delete_post_tags_query);
    if ($delete_post_tags_response) {
        for ($i = 0; $i < $length; $i++) {
            $user_tag_id = $user_tag_ids['user_tags_id'][$i];
            $insert_user_tags_id_query = "INSERT INTO `wo_knkt_post_tags`(`post_id`, `user_id`, `status`) VALUES ('$post_id','$user_tag_id',1)";
            mysqli_query($sqlConnect, $insert_user_tags_id_query);
...
function get_post_by_interest_id($post_data)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
...
function invokeApi($api_method, $url, $data = '')
{
    global $wo;
    // $api_key = constant("X-Api-Key");
    $url = $wo['config']['site_url'] . '/' . $url;
    // echo $url;die;
    $curl = curl_init($url);
...
function invokeApi_test($api_method, $url, $data = '')
{
    global $wo;
    // $api_key = constant("X-Api-Key");
    $url = $wo['config']['site_url'] . '/' . $url;
...
function Wo_user_interest_existe($user_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $user_interest_exist_query = "SELECT * FROM `wo_user_interest` WHERE `user_id`=$user_id";
    $user_interest_query_result = mysqli_query($sqlConnect, $user_interest_exist_query);
    if (mysqli_num_rows($user_interest_query_result) > 0) {
        return array('status' => "1", "data" => "User Exist");
    } else {
        return array('status' => "0", "data" => "User Not Exist");
    }
}
function Wo_edit_status_post($post_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    if (!empty($post_id)) {
        $edit_post_status_query = "UPDATE `wo_knkt_post` SET `status`='1' WHERE `id`=$post_id";
        $edit_post_status_result = mysqli_query($sqlConnect, $edit_post_status_query);
        if ($edit_post_status_result == 1) {
            return array('status' => "1", "data" => "status_update");
        } else {
            return array('status' => "0", "data" => "status_not_update");
        }
    } else {
        return array('status' => "0", "data" => "post_id_empty");
    }
}
function Wo_post_list($post_type, $question = "", $city = "")
{
    global $sqlConnect, $wo;
    $login_user_id = $wo['user']['user_id'];
    $post_list = array();
    ...
    if ($post_type == "all") {
        $post_list_query = "SELECT * FROM `wo_knkt_post` WHERE `status`='1' ORDER BY id DESC";
    } else if ($question) {
        if ($city != "") {
        ...
        $post_list_query = "SELECT * FROM `wo_knkt_post` WHERE `status`='1' and share=0 and `post_describtions`LIKE '%$question%' $query ORDER BY id DESC";
    } else if ($post_type > 0) {
        $post_list_query = "SELECT p.* FROM `wo_knkt_post`p LEFT OUTER JOIN wo_knkt_post_interest pi on (p.id=pi.post_id) WHERE pi.interest_id=$post_type and p.status=1 ORDER BY pi.id DESC";
    }
    if ($post_type == "recent") {
        $today_date = date("Y-m-d");
        $post_list_query = "SELECT * FROM `wo_knkt_post` WHERE `status`='1' and `date`='$today_date' ORDER BY id DESC";
    }

    $post_list_result = mysqli_query($sqlConnect, $post_list_query);
    if (mysqli_num_rows($post_list_result) > 0) {
        while ($row = mysqli_fetch_assoc($post_list_result)) {
            ...
            $share = $row['share'];
            if ($share == '1') {
                $share_post_query = "SELECT u.`first_name`,u.`last_name`,u.avatar FROM `Wo_Users` u LEFT OUTER JOIN `wo_knkt_share_post` sp on(sp.creator_id=user_id) WHERE sp.post_id='$post_id'";
                $share_post_result = mysqli_query($sqlConnect, $share_post_query);
                $share_post = mysqli_fetch_all($share_post_result, MYSQLI_ASSOC);
                $row['share_post'] = $share_post;
            }
            $post_interest_query = "SELECT pi.*, i.name as interest_name FROM `wo_knkt_post_interest` pi LEFT OUTER JOIN wo_interest i on (pi.interest_id=i.id) WHERE pi.post_id='$post_id'";
            $post_interest_result = mysqli_query($sqlConnect, $post_interest_query);
            $post_interest = mysqli_fetch_all($post_interest_result, MYSQLI_ASSOC);
            $row['post_interest'] = $post_interest;
            $user_data_query = "SELECT * FROM `Wo_Users` WHERE `user_id`='$user_id'";
            $user_data_result = mysqli_query($sqlConnect, $user_data_query);
            if (mysqli_num_rows($user_data_result) > 0) {
                $user_data = mysqli_fetch_object($user_data_result);
                $row['user_data'] = $user_data;
            }
            $like_count_query = "SELECT count(*)  FROM `wo_knkt_post_like` WHERE `post_id`='$post_id'";
            $user_data_result = mysqli_query($sqlConnect, $like_count_query);
            if (mysqli_num_rows($user_data_result) > 0) {
                while ($like_count_row = mysqli_fetch_array($user_data_result)) {
                    $row['post_like_count'] = $like_count_row[0];
                }
            }
            $post_view_count_query = "SELECT count(*) FROM `wo_knkt_view_post` WHERE `post_id`='$post_id'";
            $post_view_count_result = mysqli_query($sqlConnect, $post_view_count_query);
            if (mysqli_num_rows($post_view_count_result) > 0) {
                while ($view_count = mysqli_fetch_array($post_view_count_result)) {
                    $row['view_count'] = $view_count[0];
                }
            }
            $like_list_query = "SELECT *  FROM `wo_knkt_post_like` WHERE `post_id`='$post_id' and `user_id`='$login_user_id'";
            $like_list_result = mysqli_query($sqlConnect, $like_list_query);
            ...
            $post_media_query = "SELECT * FROM `wo_knkt_post_media` WHERE `post_id`='$post_id'";
            $post_media_result = mysqli_query($sqlConnect, $post_media_query);
            if (mysqli_num_rows($post_media_result) > 0) {
                while ($media_row = mysqli_fetch_assoc($post_media_result)) {
                    $row['media'] = $media_row;
                }
            }
            $post_list[] = $row;
        }
        return array('status' => "1", 'data' => $post_list);
    } else {
        return array('status' => "0", 'data' => $post_list);
    }
}
function Wo_most_like_post_list()
{
    global $sqlConnect, $wo;
    $post_list = array();
    if ($wo['loggedin'] == false) {
    ...
    $post_list_query = "SELECT kp.* ,count(kp.id) as `post_like_count` from wo_knkt_post kp LEFT OUTER JOIN wo_knkt_post_like pl ON pl.post_id = kp.id group by kp.Id HAVING COUNT(kp.id) > 3";
    $post_list_result = mysqli_query($sqlConnect, $post_list_query);
    if (mysqli_num_rows($post_list_result) > 0) {
        while ($row = mysqli_fetch_assoc($post_list_result)) {
            if ($row['post_like_count'] > "3") {
                $post_id = $row['id'];
                $user_id = $row['user_id'];
                $post_interest_query = "SELECT pi.*, i.name as interest_name FROM `wo_knkt_post_interest` pi LEFT OUTER JOIN wo_interest i on (pi.interest_id=i.id) WHERE pi.post_id='$post_id'";
                $post_interest_result = mysqli_query($sqlConnect, $post_interest_query);
                ...
                $like_list_query = "SELECT *  FROM `wo_knkt_post_like` WHERE `post_id`='$post_id'";
                $like_list_result = mysqli_query($sqlConnect, $like_list_query);
                if (mysqli_num_rows($like_list_result) > 0) {
                    $like_data = mysqli_fetch_object($like_list_result);
                    $row['like_data'] = $like_data;
...
function Wo_post_like($post_like, $post_id, $like_id = 0)
{
    ...
    if ($post_like == "1") {
        $post_like_query = "INSERT INTO `wo_knkt_post_like`(`post_id`, `user_id`) VALUES ('$post_id','$user_id')";
        $post_like_result = mysqli_query($sqlConnect, $post_like_query);
        if ($post_like_result == '1') {
            return array('status' => "1", 'data' => "Post is Liked");
        }
    } else if ($post_like == "0") {
        $post_unlike_query = "DELETE FROM `wo_knkt_post_like` WHERE `user_id`='$user_id' and `post_id`=$post_id";
        $post_unlike_result = mysqli_query($sqlConnect, $post_unlike_query);
        if ($post_unlike_result) {
            return array('status' => "0", 'data' => "Post is UnLiked");
        }
    }
}
function Wo_post_comment($post_id, $comment)
{
    global $sqlConnect, $wo;
    $user_id = $wo['user']['user_id'];
    $post_like_query = "";
    ...
    $create_comment_query = "INSERT INTO `wo_knkt_post_comments`(`comment`, `post_id`, `created_by`, `comment_id`, `status`) VALUES ('$comment','$post_id','$user_id','0','1')";
    $create_comment_result = mysqli_query($sqlConnect, $create_comment_query);
    $comment_id = mysqli_insert_id($sqlConnect);
    if ($create_comment_result == 1) {
        return array('status' => "1", 'message' => "Post Comment Succesfully Done", "data" => $comment_id);
    } else {
        return array('status' => "0", 'message' => "Post Comment Not Done");
    }
}
function Wo_post_edit_comment($comment_id, $comment)
{
    global $sqlConnect, $wo;
    $user_id = $wo['user']['user_id'];
    $post_list = array();
    ...
    $update_comment_query = "UPDATE `wo_knkt_post_comments` SET `comment`='$comment'WHERE `id`='$comment_id'";
    $update_comment_result = mysqli_query($sqlConnect, $update_comment_query);
    if ($update_comment_result == 1) {
        return array('status' => "1", 'data' => "Post Comment Edit Succesfully Done");
    } else {
        return array('status' => "0", 'data' => "Post Comment Edit Not Done");
    }
}
function Wo_post_delete_comment($comment_id)
{
    global $sqlConnect, $wo;
    $post_list = array();
    if ($wo['loggedin'] == false) {
    ...
    $delete_comment_query = "DELETE FROM `wo_knkt_post_comments` WHERE  `id`='$comment_id'";
    $delete_comment_result = mysqli_query($sqlConnect, $delete_comment_query);
    if ($delete_comment_result == 1) {
        return array('status' => "1", 'data' => "Post Comment delete Succesfully Done");
    } else {
        return array('status' => "0", 'data' => "Post Comment delete Not Done");
    }
}
function Wo_share_post($post_id)
{
    global $sqlConnect, $wo;
    $post_list = array();
    if ($wo['loggedin'] == false) {
    ...
    $get_post_query = "SELECT p.*,pm.media FROM `wo_knkt_post` p LEFT OUTER JOIN wo_knkt_post_media pm on(p.id=pm.post_id) WHERE p.id='$post_id'";
    $get_post_result = mysqli_query($sqlConnect, $get_post_query);
    if (mysqli_num_rows($get_post_result) > 0) {
        $data = mysqli_fetch_assoc($get_post_result);
        $post_title = $data['post_title'];
        $post_describtions = $data['post_describtions'];
        $user_id = $data['user_id'];
        $location = $data['location'];
        $delete_after_6_month = $data['delete_after_6_month'];
        $status = $data['status'];
        $post_like = $data['post_like'];
        $today_date = date('y-m-d');
        $media = $data['media'];

        // insert share post query
        $share_post_query = "INSERT INTO `wo_knkt_post`(`post_title`, `post_describtions`, `user_id`, `location`, `delete_after_6_month`, `share`, `status`, `date`, `post_like`) VALUES ('$post_title','$post_describtions','$user_id','$location','$delete_after_6_month','1','$status','$today_date','0')";
        $get_post_result = mysqli_query($sqlConnect, $share_post_query);
        $new_post_id = mysqli_insert_id($sqlConnect);
        if ($get_post_result) {
            $creator_id = $wo['user']['user_id'];
            // share query 
            $share_query = "INSERT INTO `wo_knkt_share_post`(`post_id`, `creator_id`) VALUES ('$new_post_id','$creator_id')";
            $share_query_result = mysqli_query($sqlConnect, $share_query);
            // get post_interst 
            $get_post_interest = "SELECT * FROM `wo_knkt_post_interest` WHERE `post_id`='$post_id'";
            $get_post_interest_result = mysqli_query($sqlConnect, $get_post_interest);
            while ($row = mysqli_fetch_assoc($get_post_interest_result)) {
                $interest_id = $row['interest_id'];
                $share_post_interest = "INSERT INTO `wo_knkt_post_interest`(`post_id`, `interest_id`) VALUES ('$new_post_id','$interest_id')";
                mysqli_query($sqlConnect, $share_post_interest);
            }
        }
        // share media query
        $post_media = "INSERT INTO `wo_knkt_post_media`(`post_id`, `media`) VALUES ('$new_post_id','$media')";
        $get_post_result = mysqli_query($sqlConnect, $post_media);
        if ($get_post_result) {
            return array('status' => "1", 'data' => "Share Post Create");
        }
    } else {
        return array('status' => "0", 'data' => "Share Post Not Create");
    }
}
function Wo_interest_id($user_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    ...
    $user_interest = array();
    $user_interest_query = "SELECT * FROM `wo_user_interest` WHERE `user_id`='$user_id';";
    $user_interest_result = mysqli_query($sqlConnect, $user_interest_query);
    if (mysqli_num_rows($user_interest_result) > 0) {
        $user_interest = mysqli_fetch_all($user_interest_result, MYSQLI_ASSOC);
        return array('status' => "1", "data" => $user_interest);
    } else {
        return array('status' => "0", 'data' => $user_interest);
    }
}
function add_feedback($name, $email, $query_type, $query)
{
    global $sqlConnect, $wo;
    ...
    $add_feedback_query = "INSERT INTO `wo_feedback`( `name`, `email`, `query_type`, `query`) 
    VALUES ('$name','$email','$query_type','$query') ";
    $add_feedback_result = mysqli_query($sqlConnect, $add_feedback_query);
    if ($add_feedback_result >= 1) {
        return array('status' => '1', 'data' => 'inserted feedback');
    } else {
        return array('status' => '0', 'data' => 'not inserted feedback');
    }
}
function create_wo_groups($name, $profile_image, $cover_image, $short_discription, $detail_discription, $group_type, $role_permission = "admin")
{

    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $profile_image_endpoint = $cover_image_endpoint = "";
    if ($wo['loggedin'] == false) {
        return false;
    }
    ...
    if (isset($profile_image)) {
        ...
        $profile_end_url = Wo_ShareFile($fileInfo, 0);
        $profile_url = $profile_end_url['filename'];
    }
    if (isset($cover_image)) {
        $fileInfo = array(
            'file' => $cover_image["tmp_name"],
            'name' => $cover_image['name'],
            'size' => $cover_image["size"],
            'type' => $cover_image["type"],
        );

        $cover_end_url = Wo_ShareFile($fileInfo, 0);
        $cover_url = $cover_end_url['filename'];
    }
    $wo_groups_query = "INSERT INTO `wo_knktn_groups`(`name`, `profile_image`, `cover_image`, `short_discription`, `detail_discription`, `group_type`) VALUES ('$name','$profile_url','$cover_url','$short_discription','$detail_discription','$group_type')";
    $wo_groups_result = mysqli_query($sqlConnect, $wo_groups_query);
    $group_id = mysqli_insert_id($sqlConnect);

    $wo_group_permission_query = "INSERT INTO `wo_knktn_groups_permission`( `group_id`, `user_id`, `role_permission`) VALUES ('$group_id','$user_id','$role_permission')";
    $wo_group_permission_query_result = mysqli_query($sqlConnect, $wo_group_permission_query);
    if ($wo_groups_result == "1") {
        return array('status' => '1', 'data' => 'Group Created');
    } else {
        return array('status' => '0', 'data' => 'Group Not Created');
    }
}

function update_wo_groups($id, $name, $profile_image, $cover_image, $short_discription, $detail_discription, $group_type)
{
    global $sqlConnect, $wo;
    $profile_image_endpoint = $cover_image_endpoint = "";
    if ($wo['loggedin'] == false) {
    ...
    if (isset($profile_image)) {
        ...
        $profile_end_url = Wo_ShareFile($fileInfo, 0);
        $profile_url = $profile_end_url['filename'];
        $profileImage = '`profile_image`' . '=' . "'" . $profile_url . "'" . ',';
    } else {
        $profileImage = "";
    }
    if (isset($cover_image)) {
        ...
        $cover_end_url = Wo_ShareFile($fileInfo, 0);
        $cover_url = $cover_end_url['filename'];
        $coverImage = '`cover_image`' . '=' . "'" . $cover_url . "'" . ',';
    } else {
        $coverImage = "";
    }
    $wo_groups_update_query = "UPDATE `wo_knktn_groups` SET `name`='$name',$coverImage $profileImage`short_discription`='$short_discription',`detail_discription`='$detail_discription',`group_type`='$group_type' WHERE id = '$id'";

    $wo_groups_update_result = mysqli_query($sqlConnect, $wo_groups_update_query);
    if ($wo_groups_update_result == "1") {
        return array('status' => '1', 'data' => 'Group updated');
    } else {
        return array('status' => '0', 'data' => 'Group updated');
    }
}

function delete_wo_groups($id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $wo_groups_delete_query = "DELETE FROM `wo_knktn_groups` WHERE id = '$id' ";
    $wo_groups_delete_result = mysqli_query($sqlConnect, $wo_groups_delete_query);
    if ($wo_groups_delete_result == "1") {
        $delete_group_permission = "DELETE FROM `wo_knktn_groups_permission` WHERE `group_id`='$id'";
        mysqli_query($sqlConnect, $delete_group_permission);
        return array('status' => '1', 'data' => 'Group deleted');
    } else {
        return array('status' => '0', 'data' => 'Group deleted');
    }
}
function list_wo_groups($group_type = 0)
{
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $data = array();
    ...
    if ($group_type == 0) {
        $wo_groups_list_query = " SELECT * FROM `wo_knktn_groups` WHERE `group_type`='$group_type' ";
    } else {
        $wo_groups_list_query = "SELECT kg.* FROM `wo_knktn_groups_permission` kgp LEFT OUTER JOIN wo_knktn_groups kg on(kgp.`group_id`=kg.id) WHERE kgp.user_id='$user_id' AND `group_type`=1";
    }
    $wo_groups_list_result = mysqli_query($sqlConnect, $wo_groups_list_query);
    if (mysqli_num_rows($wo_groups_list_result) > 0) {
        while ($row = mysqli_fetch_assoc($wo_groups_list_result)) {
            $group_id = $row['id'];
            $user_exist_in_the_group = "SELECT * FROM `wo_knktn_groups_permission` WHERE `user_id`='$user_id' AND `group_id`='$group_id'";
            $user_exist_in_the_result = mysqli_query($sqlConnect, $user_exist_in_the_group);
            $user_exist_data = mysqli_fetch_all($user_exist_in_the_result, MYSQLI_ASSOC);
            $row['user_data'] = $user_exist_data;
            $data[] = $row;
        }
...
function group_detail($group_id)
{
    ...
    $detail_group_query = "SELECT * FROM `wo_knktn_groups` WHERE `id`='$group_id'";
    $detail_group_result = mysqli_query($sqlConnect, $detail_group_query);
    if (mysqli_num_rows($detail_group_result) > 0) {
        $data = mysqli_fetch_object($detail_group_result);
        return array('status' => '1', 'data' => $data);
    } else {
        return array('status' => '0', 'data' => $data);
    }
...
function join_group($group_id)
{
    ...
    $join_group_query = "INSERT INTO `wo_knktn_groups_permission`(`group_id`, `user_id`, `role_permission`) VALUES ('$group_id','$user_id','member')";
    $join_group_result = mysqli_query($sqlConnect, $join_group_query);
    if ($join_group_result == "1") {
        return array('status' => '1', 'data' => 'Join Group Succesfully');
    } else {
        return array('status' => '0', 'data' => 'Group Not Join');
    }
}
function member_exist($group_id)
{
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $data = array();
    ...
    $member_exist_query = "SELECT * FROM `wo_knktn_groups_permission` WHERE `user_id`='$user_id' and group_id='$group_id'";
    $join_group_result = mysqli_query($sqlConnect, $member_exist_query);
    if (mysqli_num_rows($join_group_result) > 0) {
        return array('status' => '1', 'data' => 'You Are Already member of this group');
    }
}
function user_group_list($user_id)
{
    global $sqlConnect, $wo;
    $data = array();
    if ($wo['loggedin'] == false) {
    ...
    $user_group_list = "SELECT kg.*,kgp.role_permission FROM `wo_knktn_groups_permission` kgp LEFT OUTER JOIN wo_knktn_groups kg on(kgp.`group_id`=kg.id) WHERE kgp.user_id='$user_id'";
    $user_group_list_result = mysqli_query($sqlConnect, $user_group_list);
    if (mysqli_num_rows($user_group_list_result) > 0) {
        while ($row = mysqli_fetch_assoc($user_group_list_result)) {
            $group_id = $row['id'];
            $get_recent_message = "SELECT * FROM `message_sender` WHERE `group_id`='$group_id' ORDER  BY `id` DESC
            LIMIT  1";
            $get_recent_message_result = mysqli_query($sqlConnect, $get_recent_message);
            $get_recent_message_data = mysqli_fetch_object($get_recent_message_result);
            $row['last_message'] = $get_recent_message_data;
            $data[] = $row;
        }
...
function create_groups_messages($message, $group_id, $time, $get_image)
{
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $user_name = '@' . $wo["user"]['first_name'];
    $data = array();
    if ($wo['loggedin'] == false) {
        return false;
    }
    $get_user_permission_query = "SELECT  `role_permission` FROM `wo_knktn_groups_permission` WHERE `user_id`='$user_id'";
    $get_user_permission_result = mysqli_query($sqlConnect, $get_user_permission_query);
    $user_permission_data = mysqli_fetch_object($get_user_permission_result);
    $role = $user_permission_data->role_permission;
    $get_admin_id_query = "SELECT `user_id` FROM `wo_knktn_groups_permission` WHERE `group_id`='$group_id' and `role_permission`='admin'";
    $get_admin_id_result = mysqli_query($sqlConnect, $get_admin_id_query);
    $get_admin_id = mysqli_fetch_object($get_admin_id_result);
    $group_admin_id = $get_admin_id->user_id;
    $create_chat_query = "INSERT INTO `message_sender`(`message`,`sender_id`, `group_id`, `sender_role`,`user_name`,`time`,`sender_image`,`admin_id`) VALUES ('$message',
'$user_id','$group_id','$role','$user_name','$time','$get_image','$group_admin_id')";
    $create_chat_result = mysqli_query($sqlConnect, $create_chat_query);
    $last_message_id = mysqli_insert_id($sqlConnect);
    if ($create_chat_result) {
        $get_group_members = "SELECT * FROM `wo_knktn_groups_permission` WHERE `group_id`='$group_id' and `user_id`!='$user_id'";
        $get_group_members = mysqli_query($sqlConnect, $get_group_members);
        if (mysqli_num_rows($get_group_members) > 0) {
            while ($rows = mysqli_fetch_assoc($get_group_members)) {
                $group_member_id = $rows['user_id'];
                $create_reciver_query = "INSERT INTO `receiver_messages`( `receiver_id`, `messages_id`) VALUES ('$group_member_id','$last_message_id')";
                mysqli_query($sqlConnect, $create_reciver_query);
            }
        }
        return array('status' => '1', 'data' => 'Message sent');
    } else {
        return array('status' => '0', 'data' => $data);
    }
}
function group_chat_list($group_id)
{
    global $sqlConnect, $wo;
    $user_id = $wo["user"]["id"];
    $data = array();
    ...
    $groups_chat_list_query = "SELECT * FROM `message_sender` WHERE `group_id`='$group_id'ORDER by id ASC";
    $groups_chat_list_result = mysqli_query($sqlConnect, $groups_chat_list_query);
    if (mysqli_num_rows($groups_chat_list_result) > 0) {
        while ($row = mysqli_fetch_assoc($groups_chat_list_result)) {
            $data[] = $row;
        }
        return array('status' => '1', 'data' => $data);
    } else {
...
function delete_group_message($message_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $delete_message_query = "DELETE FROM `message_sender` WHERE `id`='$message_id'";
    $delete_message_result = mysqli_query($sqlConnect, $delete_message_query);
    if ($delete_message_result == 1) {
        $delete_recieve_message_query = "DELETE FROM `receiver_messages` WHERE `messages_id`='$message_id'";
        mysqli_query($sqlConnect, $delete_recieve_message_query);
        return array('status' => "1", 'data' => "Delete message delete Succesfully Done");
    } else {
        return array('status' => "0", 'data' => "Delete message delete Not Done");
    }
}
function create_post_view($user_id, $post_id)
{
    global $sqlConnect, $wo;
    if ($wo['loggedin'] == false) {
        return false;
    }
    $view_exist_query = "SELECT * FROM `wo_knkt_view_post` WHERE `post_id`='$post_id' and `user_id`='$user_id'";
    $view_exist_result = mysqli_query($sqlConnect, $view_exist_query);
    if (mysqli_num_rows($view_exist_result) > 0) {
        return array('status' => "0", 'data' => "User aleready view in this post");
    } else {
        $insert_view_query = "INSERT INTO `wo_knkt_view_post`(`post_id`, `user_id`) VALUES ('$post_id','$user_id')";
        $view_exist_result = mysqli_query($sqlConnect, $insert_view_query);
        if ($view_exist_result) {
            return array('status' => "1", 'data' => "User view in this post");
        } else {
            return array('status' => "0", 'data' => "User not view in this post");
        }
    }
}