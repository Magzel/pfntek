...
function get_list($sqlConnect,$table_name)
{
    $get_info = array();
    ...
    if (mysqli_num_rows($query_result) == 0) {
        ...
        $get_info = mysqli_fetch_all($query_result, MYSQLI_ASSOC);
        return array('status' => "1", 'get_info' => $get_info);
    }
}
function get_data_by_id($sqlConnect, $table_name,$id)
{
    $get_info = array();
    $faq_list = "SELECT * FROM $table_name WHERE id='$id'";
    $get_faq_list_info_result = mysqli_query($sqlConnect, $faq_list);
    if (mysqli_num_rows($get_faq_list_info_result) == 0) {
        return array('status' => "0", 'get_info' => $get_info);
    } else {
        $get_info = mysqli_fetch_array($get_faq_list_info_result);
        return array('status' => "1", 'get_info' => $get_info);
    }
}
?>
